...
;(def get-crumb "7w0Ak6e2MVc") ;(slurp "https://query2.finance.yahoo.com/v1/test/getcrumb") npzge7VVxp0

(def query-head "https://query1.finance.yahoo.com/v8/finance/chart/")
(def query-tail "?metrics=close?&interval=1d")               ;&range=max     "&crumb=7w0Ak6e2MVc"

(defn get-yahoo-last-price [list-ticker]
  "Extract latest price from yahoo finance"
  (let [data (flatten
                  (for [ticker list-ticker]
                    (let [results (first (vals (first (get-in (cheshire.core/parse-string (slurp (str query-head ticker query-tail "&range=1d"))) ["chart" "result" ]))))
                          currency (get results "currency")
                          exchange-name (get results "exchangeName")
                          last-close-price (get results "chartPreviousClose")]
                      (into {}
                            {:ticker ticker :last-close-price last-close-price :currency currency :exchange-name exchange-name}))))
        ]
    data
    )
  )

;(defn read-raw-response [raw seq] (get-in (t/json->kmap (:body raw)) (if (keyword? seq) [seq] seq)))
;(read-raw-response
;  @(http/get "https://query2.finance.yahoo.com/v10/finance/quoteSummary/AAPL?modules=price&crumb=npzge7VVxp0"
;             {:headers {                                    ;"cookie" "louter=i13n"
...
;25 requests per day...
; premium is .... 25USd per month for 30 API requests per minutes
; need create a cash, refresh everyday within limit

;(def key-test "AHDO7I12UJI7TITY")
;(def ticker1 "TTE.PA")
;(def ticker2 "TTE")
;(def data-type "json") ;or csv
;(def endpoint-timeseries-daily (str "https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=" ticker1 "&apikey=" key-test)) ;daily is not adjusted, we only use it for last price, with exchange suffix
;(def endpoint-timeseries-weekly (str "https://www.alphavantage.co/query?function=TIME_SERIES_WEEKLY&symbol=" ticker2 "&apikey=" key-test "&datatype=" data-type)) ;adjusted price requires PREMIUM...
;(def endpoint-overview (str "https://www.alphavantage.co/query?function=OVERVIEW&symbol=" ticker2 "&apikey=" key-test))
;
...
;
;(def list-ticker ["AAPL" "TTL"] )
;
;(defn get-last-price [list-ticker]
;  "Extract latest price from yahoo finance - used for FX and metals"
;  (let [result (flatten
;                  (for [ticker list-ticker]
;                    (let [response (get-in (cheshire.core/parse-string (slurp endpoint-timeseries-daily)) ["Time Series (Daily)"])
;                          dates (keys response)
;                          max_date "dsdsd"
;                          results (get response [max_date])
;                          ]
;                      (into {}
;                            {:ticker ticker :value (get response "raw")}))))
;        ]
;    result
;    )
;  )
